...
FROM conan_build_env as conan_flatc_repoadd_source_install
...
ARG CONAN="conan"
ARG PKG_NAME="conan_flatc/v1.11.0"
ARG PKG_CHANNEL="conan/stable"
ARG PKG_UPLOAD_NAME="conan_flatc/v1.11.0@conan/stable"
...
RUN set -ex \
  ...
  export CXX=g++ \
  && \
  $CONAN remote add google_aardappel_flatbuffers https://api.bintray.com/conan/aardappel/flatbuffers False \
  && \
